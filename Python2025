...
spare_files = np.array(["dark_sky_data/Spare1.csv",
                      "dark_sky_data/Spare2.csv"])

spare_files_processed = np.array(["dark_sky_data/Spare1_processed.csv",
                      "dark_sky_data/Spare2_processed.csv"])
...
for i in range(len(spare_files)):
    with open(spare_files[i], "a", newline="") as f:
        writer = csv.writer(f, delimiter=",")
        writer.writerow([1,0])
        
for i in range(len(spare_files_processed)):
    with open(spare_files_processed[i], "a", newline="") as f:
        writer = csv.writer(f, delimiter=",")
        writer.writerow(['Runner','Time(s)', 'Time(mm:ss)'])
...
while True:
    try:
        ...
        elif x[0] in range(8,15):
            void_lap(x)
            print('LAP VOIDED')
            
        elif x[0] in range(15,17):
            
            house_id = int(x[0]) - 15
            with open(spare_files[house_id], "a", newline="") as f:
                writer = csv.writer(f, delimiter=",")
                writer.writerow(x)

            with open(str(spare_files[house_id]), "r") as f:
                lines = f.readlines()
                last_line = lines[-2].strip()
        
            last_val = last_line[2:len(last_line)+1]
          
            true_time = (float(x[1])-float(last_val))/1000
           
            
            with open(spare_files_processed[house_id], "a", newline="") as f:
                writer = csv.writer(f, delimiter=",")
                writer.writerow(["name_here", true_time,str(datetime.timedelta(seconds=true_time))[2:10]])
